...
public class ManualMode extends PlayMode{
    ...
    ArrayList<Integer> gameScore = new ArrayList<>();
    int countOfHints;
    int countOfSteps;
    ...
    WordsDictionary dictionary = new WordsDictionary("src/main/java/words_alpha.txt");

    List<String> list = dictionary.getWords();

    Map<String, Users> userList = new HashMap<String, Users>();

    private ArrayList<Stats> listOfGameStatsPerUniqueUser = new ArrayList<>();
    Map<InfinityKey, ArrayList<Stats>> gameMap = new HashMap<>();
    ...
    public ManualMode() throws IOException {
...
//        listOfPlayerNames.add(username);
        ...
        do {
            ...
            if(errorsForFirstWord == null){
                ...
                Stats gameStats = new Stats(countOfHints, countOfSteps, initialMostEfficientPath, myPath, runtimeOfApp);
                listOfGameStatsPerUniqueUser.add(gameStats);
                gameMap.put(new InfinityKey(), listOfGameStatsPerUniqueUser);

                nrOfGames = listOfGameStatsPerUniqueUser.size();
                Users userStats = new Users(nrOfGames);
                userList.put(username, userStats);

                for(Stats stats : listOfGameStatsPerUniqueUser ){
                    database.addStatsToCSV(stats);
                }
                for(Stats stats : listOfGameStatsPerUniqueUser ) {
                    database.writeToCsv(stats);
                }
